...
* xref:configuration:features.adoc#bookkeeper-autoscaler[Bookkeeper autoscaler] - Automatically scale the number of bookies based on memory usage.
* xref:configuration:features.adoc#broker-autoscaler[Broker autoscaler] - Automatically scale the number of brokers based on CPU load.
* xref:configuration:features.adoc#rackaware-bookie[Rack-aware bookkeeper placement] - Place bookies in different racks to guarantee high availability.
* xref:configuration:features.adoc#resource-sets[Resource sets] - Create multiple distinct sets of Pulsar components, each with a dedicated deployment/StatefulSet with its own service and configmap.
