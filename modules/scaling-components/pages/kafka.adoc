...
Have an Apache Kafka(R) workload you want to control with {pulsar-operator}?
Thanks to Starlight for Kafka, you can run your Kafka workload on a {pulsar-short} cluster, and with {pulsar-operator}, the scaling of the Kafka pods is handled for you.

== Scale the {pulsar-short} Broker with a Kafka Client Workload
...
In this case, we're installing xref:getting-started:stack.adoc[{pulsar-short} Stack] with the Kafka protocol enabled.
+
[source,shell]
----
...
----
+
Additionally, you can proxy the Kafka connection in the {pulsar-short} Proxy with `kafka:enabled:true`.
+
[source,yaml]
----
proxy:
