...
The CPU usage of each broker is checked at the Pulsar load balancer, not just at the Kubenetes pod level.
...
. Run a https://pulsar.apache.org/docs/performance-pulsar-perf/[Pulsar perf] test in your deployment, and then follow the operator's logs to see the autoscaler in action:
+
[source,shell]
----
