...
The {product-short} Helm chart installs only the operator pod and the `PulsarCluster` custom resource definition (CRD).

Alternatively, you can xref:getting-started:stack.adoc[install {pulsar-stack}], which includes the operator, CRD, and a full Prometheus monitoring stack.
...
Use Helm to install {product-short}:
...
. The {product-short} Helm chart is available for download (https://github.com/datastax/kaap/releases/latest)[here].
...
. Ensure {product-short} is up and running:
+
[source,shell]
----
