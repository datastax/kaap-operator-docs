...
{product} 0.2.0 is a major release that includes new {crd} versions.
This means that you must migrate your existing v0.1.0 {crd-short}s to v0.2.0 {crd-short}s.

[IMPORTANT]
====
The {crd-short} specs are fully compatible, but you _must_ change the existing {crd-short} version to `v1beta1` during the upgrade.
====
...
. Apply the new {crd-short} versions.
+
This process will update the {crd-short} by adding a new version `v1beta1`.
+
[source,shell]
----
...
. Copy the KAAP {cr-short}s and move them to `v1beta1`:
+
[source,shell]
----
